...
Usage:
  python scripts/verify.py --yaml doid.yaml
...
def main() -> None:
    ...
    if broken_parents:
        ...
        nmore = len(broken_parents) - len(preview)
        more = f" (+{nmore} more)" if len(broken_parents) > len(preview) else ""
        errors.append(f"broken parent refs ({len(broken_parents)}): {preview}{more}")

    term_count = len(terms)
    print(
        "verify: "
        f"terms={term_count}, unique_ids={len(known)}, "
        f"broken_parent_refs={len(broken_parents)}"
    )
    if errors:
